...
//     let v1 = vec![0, 1, 2];
//     let v2 = vec![3, 4, 5];
//     let v1_clone = v1.clone(); //Tao 1 clone de truyen vao diff2()
//     let diff = vec_diff(v2, v1);
//     let diff_clone = diff.clone(); //Tao 1 clone de truyen vao macro println!()
//     let diff2  = vec_diff(diff_clone, v1_clone);
//     println!("{:?}", all(diff, 3));
// }
...
//     while additions_clone.len() > 0 { //Get length from a clone instead
//         let mut addition: f64 = 0.0;
//         let mut saltar: i32 = 0;

//         // Sumar valores en additions
//         for element_index in &additions  {//Must use reference because value is borrowed every loop
//             let addition_aux = values[*element_index]; //Derefenrencing value
...
//     let num_str_clone = num_str.clone();
//     let chars = num_str_clone.chars();
//     let chars_clone = chars.clone();// Create clone of chars because it's borrowed 1 time below
//     let len = chars_clone.count();  
...
fn go(v: &mut Vec<i32>) {
    for i in &*v {     //Need to get a "reference of 'dereference value' of v" to iterate
        println!("{}", i);
    }
    v.push(4);
}